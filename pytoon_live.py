...
Chat mode (Ollama replies, avatar speaks the reply):
    ...
    python pytoon_live.py --chat --ollama-model llama3.1:8b  # or gemma4:e2b
...
from agents import classify_intent, task_agent, reminder_agent, chat_agent
try:
    from crud_tools import TOOLS as CRUD_TOOLS, TOOL_FUNCS as CRUD_TOOL_FUNCS, get_tool_descriptions
except Exception:
            ...
            if getattr(args, "reset_memory", False):
                for p in (getattr(args, "memory_file", None) or DEFAULT_HISTORY_FILE, facts_path, mem_path):
                    try:
                        ...
                        pass
                from memory_handler import reset_all
                try:
                    reset_all()
                except Exception as e:
...
def main(argv=None):
    ...
    p.add_argument("--ollama-model", default="llama3.2:1b", choices=["llama3.2:1b", "qwen3:4b", "llama3.1:8b", "gemma4:e2b"],
                   help="Ollama model for --chat (default llama3.2:1b: fastest clean answers on Pi CPU; qwen3 narrates its reasoning)")
    p.add_argument("--system", default=(
        ...
        "NEVER output thinking, reasoning, <think> or <thinking> tags, or any internal monologue — only the final human reply.\n"
