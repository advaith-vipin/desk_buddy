...
Examples:
    python -m desk_buddy.pytoon_cli --text "Hello, welcome to my application!"
    python -m desk_buddy.pytoon_cli --script script.txt --output out.mp4
    python -m desk_buddy.pytoon_cli --audio speech.mp3 --transcript "audio words here"
    python -m desk_buddy.pytoon_cli --audio speech.mp3                   # auto transcript
"""
...
# Package bootstrap: allow running this file directly (python .../pytoon_cli.py)
...
from ..prompts import SYSTEM_PROMPT, CLI_MEMORY_BLOCK
...
def chat_with_memory(user_text: str, system_prompt: str | None = None, model: str = "llama3.1:8b") -> str:
    ...
    final_system = (system_prompt or SYSTEM_PROMPT).replace("{current_datetime}", current_dt)
    ctx = get_memory_context()
    if ctx:
        final_system = final_system + CLI_MEMORY_BLOCK.format(context=ctx)
    # 3. Call Ollama
    import ollama
...
def main(argv=None):
    ...
    if args.forward:
        from ..avatar.front_face import forward_export
