...
from . import crud_tools as _crud
from .crud_tools import TOOLS, TOOL_FUNCS  # re-export for LLM tool calling

from .._paths import TMP_DIR
from ..prompts import INTENT_CLASSIFY_PROMPT, TASK_EXTRACT_PROMPT, REMINDER_EXTRACT_PROMPT, JSON_SUFFIX

LOG = os.path.join(TMP_DIR, "intent.log")
...
def classify_intent(prompt, model="gemma4:e2b"):
    ...
    try:
        import ollama
        resp = ollama.chat(model=model, messages=[{"role": "user", "content": INTENT_CLASSIFY_PROMPT.format(prompt=prompt)}], options={"num_predict": 5, "num_ctx": 256, "keep_alive": "10m"})
...
def _extract_llm_json(prompt, model, instruction):
    try:
        ...
        resp = ollama.chat(model=model, messages=[{"role": "user", "content": instruction + JSON_SUFFIX.format(prompt=prompt)}], options={"num_predict": 60, "num_ctx": 256, "keep_alive": "10m"})
...
def task_agent(prompt, history, model="gemma4:e2b"):
    data = _extract_llm_json(prompt, model, TASK_EXTRACT_PROMPT)
...
def reminder_agent(prompt, history, model="gemma4:e2b"):
    data = _extract_llm_json(prompt, model, REMINDER_EXTRACT_PROMPT)
    act = (data.get("action") or "").lower()
    plow = prompt.lower()
